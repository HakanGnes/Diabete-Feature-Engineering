...
def load():
    data = pd.read_csv("measurement_problems/datasets/diabetes.csv")
    return data
...
# Hiç bir işlem yapılmadan elde edilecek skoru elde etmek için bir base model oluşturuyoruz.
# Deneme Modeli
y = df["Outcome"]
...
# Aykırı Değerleri Baskılama İşlemi
...
df['NEW_BMI'] = pd.cut(x=df['BMI'], bins=[0, 18.5, 24.9, 29.9, 34.9, 100],labels=["Underweight", "Healthy", "Overweight", "Obese", "Extremely Obese"])
...
df.loc[((df["BMI"] >= 30) & (df["BMI"] < 34.9)) & ((df["Age"] >= 21) & (df["Age"] < 50)), "NEW_AGE_BMI_NOM"] = "obesemature"
df.loc[((df["BMI"] >= 30) & (df["BMI"] < 34.9)) & (df["Age"] >= 50), "NEW_AGE_BMI_NOM"] = "obesesenior"
df.loc[(df["BMI"] > 35) & ((df["Age"] >= 21) & (df["Age"] < 50)), "NEW_AGE_BMI_NOM"] = "extremeobesemature"
df.loc[(df["BMI"] > 35) & (df["Age"] >= 50), "NEW_AGE_BMI_NOM"] = "extremeobesesenior"
...
# Yeni ürettiğimiz değişkenlerin durumunu anlamak için bir fonksiyon tanımlıyoruz.
